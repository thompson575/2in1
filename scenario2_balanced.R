#---------------------------------------------------
# 20% BALANCED PLEIOTROPY SCENARIO 
#
...
#-----------------------------------------------------
# Quick MR simulation with direct pleiotropy
#
quickSimMR <- function(N,
  ...
  varGY <- 0  # var of Y due to G
  for( j in seq_len(nSNP) ) {
      Y <- Y + zeta[j]*G[, j]
      varGY <- varGY + zeta[j]*zeta[j]*varG[j]
  }
  
  varY <- sigma[2,2] + varXY + varGY   # total variance of X
  
  list(varG=varG, varX=varX, varY=varY, varGX=varGX, varXY=varXY, varGY=varGY, 
       G=G, X=X, Y=Y, G2=G2, X2=X2, af=af, sigma=sigma, alpha=alpha, beta=beta, zeta=zeta)
}
...
mrEstimators <- function(R, ap) {
  ...
  #
  bx <- bxse <- bx2 <- bxse2 <- by <- byse <- F <- F2 <- eaf <- rep(0, nSNP)
  for( i in 1:nSNP ) {
    # GX sample 1
    fit <- lm( R$X ~ R$G[,i])
  ...
  # pleiotropy measure based on simulation parameters
...
#-------------------------------------------------
# Simulation with balanced pleiotropy with same
# coef as the GX coef but independently assigned
#
quickSimData02 <- function(sp) {
  ...
  Z  <- rep(0, sp$nSNP)
  i  <- sample(1:sp$nSNP, sp$nSNP/5, replace=FALSE)
  Z[i] <- sp$a[1] + 2*abs(AF[i]-0.5)*sp$a[1] + rexp(sp$nSNP/5, rate=9/(sp$a[2]-sp$a[1]))
  Z <- (1-2*(runif(sp$nSNP)<0.5))*Z
  quickSimMR(N=sp$n, af=AF, sigma=S, alpha=A, beta=sp$beta, zeta=Z)
}
...
write_rds(E, "scenario2_results_100000.rds")
